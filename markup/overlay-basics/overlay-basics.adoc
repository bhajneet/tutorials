...
There is an html page containing all the relevant information attached to the currently activated line in Shabad OS. This html page can be manipulated by styling called CSS. -There are some default style files available- Users can also customize this page by creating their own CSS (outlined in link:../overlay-advanced/overlay-advanced.adoc[Overlay Advanced]). This html page requires javascript, which is supported by most modern web browsers.
...
TIP: One can customize these basic themes or create their own based on a copy of the given themes. See link:../overlay-advanced/overlay-advanced.adoc[Overlay Advanced] for more info.
